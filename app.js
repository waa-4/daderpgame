...
net.on("hello",p=>{if(!p.player)return;state.players.set(p.player.id,{...p.player,size:38});if(p.host)state.hostId=p.player.id;renderPlayers()});
...
function update(dt){if(!state||!me)return;let dx=0,dy=0;if(!state.draw){if(state.keys.has("a")||state.keys.has("arrowleft"))dx--;if(state.keys.has("d")||state.keys.has("arrowright"))dx++;if(state.keys.has("w")||state.keys.has("arrowup"))dy--;if(state.keys.has("s")||state.keys.has("arrowdown"))dy++;dx+=state.joy.x;dy+=state.joy.y}const l=Math.hypot(dx,dy);if(l){dx/=l;dy/=l}me.x=clamp(me.x+dx*280*dt,20,WORLD.w-20);me.y=clamp(me.y+dy*280*dt,20,WORLD.h-20);for(const[id,t]of state.targets){const p=state.players.get(id);if(p){p.x+=(t.x-p.x)*Math.min(1,dt*12);p.y+=(t.y-p.y)*Math.min(1,dt*12)}}if(performance.now()-state.lastMove>C.MOVE_INTERVAL){net.send("move",{id:me.id,x:me.x,y:me.y});state.lastMove=performance.now()}const vw=visualViewport?.width||innerWidth,vh=visualViewport?.height||innerHeight;state.cam.x=clamp(me.x-vw/2,0,Math.max(0,WORLD.w-vw));state.cam.y=clamp(me.y-vh/2,0,Math.max(0,WORLD.h-vh))}
function draw(){const vw=visualViewport?.width||innerWidth,vh=visualViewport?.height||innerHeight;ctx.clearRect(0,0,vw,vh);if(!state)return;ctx.save();ctx.translate(-state.cam.x,-state.cam.y);ctx.fillStyle="#17263d";ctx.fillRect(0,0,WORLD.w,WORLD.h);ctx.strokeStyle="#ffffff0b";for(let x=0;x<WORLD.w;x+=80){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,WORLD.h);ctx.stroke()}for(let y=0;y<WORLD.h;y+=80){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(WORLD.w,y);ctx.stroke()}ctx.fillStyle="#271a39";ctx.fillRect(DRAW.x,DRAW.y,DRAW.w,DRAW.h);ctx.strokeStyle="#ff4fc3";ctx.lineWidth=5;ctx.setLineDash([18,12]);ctx.strokeRect(DRAW.x,DRAW.y,DRAW.w,DRAW.h);ctx.setLineDash([]);ctx.fillStyle="#ffc7ee";ctx.font="bold 24px Arial";ctx.fillText("DRAWING ZONE",DRAW.x+20,DRAW.y+38);for(const s of state.strokes){ctx.beginPath();ctx.moveTo(s.x1,s.y1);ctx.lineTo(s.x2,s.y2);ctx.strokeStyle=s.color;ctx.lineWidth=s.size;ctx.lineCap="round";ctx.stroke()}for(const p of state.players.values())drawWorldPlayer(p);ctx.strokeStyle="#4a607d";ctx.lineWidth=8;ctx.strokeRect(0,0,WORLD.w,WORLD.h);ctx.restore()}
